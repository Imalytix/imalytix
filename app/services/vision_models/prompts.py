...
def detect_image_type(image_bytes: bytes) -> str:
    try:
        with Image.open(io.BytesIO(image_bytes)) as img:
            ...
            # 중간 영역(64~600): 채도 분포로 실사 vs 일러스트 구분
            # 네온 합성·HDR 편집 실사는 채도가 높지만 고유색도 많음
            # 실제 일러스트·벡터는 채도가 높고 고유색이 적음
            if unique_colors < 600:
                # 채도 평균 측정: 높은 채도 + 적은 고유색 = 일러스트
                hsv = img.convert("RGB").resize((100, 100), Image.Resampling.NEAREST).convert("HSV") if hasattr(img, "convert") else None
                try:
                    import colorsys
                    sat_values = [
                        colorsys.rgb_to_hsv(r / 255, g / 255, b / 255)[1]
                        for r, g, b in pixels[::10]  # 샘플링
                    ]
                    avg_sat = sum(sat_values) / len(sat_values) if sat_values else 0
                    # 채도 높고(>0.55) 고유색 적으면(< 300) 일러스트, 나머지는 photo
                    if avg_sat > 0.55 and unique_colors < 300:
                        return "illustration"
                    return "photo"
                except Exception:
